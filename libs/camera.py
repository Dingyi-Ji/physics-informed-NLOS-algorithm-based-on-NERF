...
class OrthographicCamera:
    ...
    def __init__(
        ...
        # ij-indices of top-left corners of pixel blocks    #这里实际上是给出了每个采样块的左上角的点
        tics = np.arange(0, res, block_size)
        j, i = np.meshgrid(tics, tics)
        self.bk_corners = np.stack([i.flatten(), j.flatten()], -1) # (b, 2)
    ...
    def _prepare_rays(self, Rt, idx=None, invert_z=False):
        # xyz-coordinates of sampled pixels in camera frame
        if idx is None:
            o = self.px_centers.reshape(-1, 3)      # 这里应该是(h*w,3)，也就是(n,3)
        else:
            o = self.px_centers[idx[:, 0], idx[:, 1]]

        # xyz-coordinates of sampled pixels in world frame
        o = np.concatenate([o, np.ones_like(o[:, :1])], -1)       # (n, 4) 创建一个与给定数组具有相同形状和数据类型的全为1的数组
        o = np.matmul(o, np.linalg.inv(Rt).T)                     # (n, 4)
        o = o[:, :3] / o[:, 3:]                                   # (n, 3)
    ...
    def sample_rays(self, Rt, batch_size=1, invert_z=False):
        ...
        # sample pixel indices  # 这里实际上是按照采样块的大小进行采样
        s = np.random.choice(self.p, (k, r))    # self.p是每个采样块中的采样点的总数
        d = np.stack([s // self.block_size, s % self.block_size], -1) # (k, r, 2)，第一个整除块内点的纵坐标，
                                                                                #第二个取余是块内的横坐标
        idx = self.bk_corners + d                                 # (k, r, 2)   # 块内点坐标加上该块的左上角顶点坐标得到该点的纵坐标
        #上面实际上实现了在每个块里面都随机采一个点，并记录index的功能
