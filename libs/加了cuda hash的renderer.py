...
from .cuda_hash_encoding import get_encoder
...
class FeatureEmbedder(nn.Module):
    """
    这里是我们模仿PosEmbedder进行编写
    用于构建一个对于特征进行编码的MLP网络结构
    输入的是一个(bs*s,c)的对于z按照空间位置进行采样的结果,输出是 (bs*s,cf的数据)
    模仿class FiLMGenerator进行编写
    """
    def __init__(
        self,
        ...
        hid_dim,           # 隐藏层的维度
        out_dim,            # 输出层的维度
        hidden_layers=2,          # 多少隐藏层，这里建议先从比较小的层开始加起
        actv = 'leaky_relu'    # 给定线性层的激活函数
    ):
        ...
        self.actv = make_actv(actv) # 建立激活函数，就是在MLP对数据进行变换之后再激活一下
    ...
    def forward(self, z_sample):
        ...
        Args:
            z_sample: 输入的是经采样的特征数据，维度为(bs*s,c)

        Returns:
            feature_nerf: 输入到nerf中的特征, 维度为(bs*s, cf)  注意这里的c和cf都是已经在yaml中设定好的
        """
        iter_number = self.hidden_layers+1
        #print(iter_number)
        #for i in range(iter_number):
        #    print("一个奇怪的问题")
        #    print(self.layers[i])
        #    z_sample = self.layers[i](z_sample)
        for now_layer in self.layers:
            #print("一个奇怪的问题")
            #print(now_layer)
            z_sample = now_layer(z_sample)

        feature_nerf = z_sample
        return feature_nerf
...
class NeRF(nn.Module):
    ...
    def __init__(
        ...
        self.d_dim = d_dim
        self.z_dim = z_dim
        self.f_t_Nerf_dim = f_t_Nerf_dim #我们加入的
        self.hid_dim = hid_dim
        self.color_dim = color_dim
        ...
        sigma_layers = []
        #in_dim = p_dim
        """注意这里需要进行修正，如果传进来了特征数据，那这里起始的维度就要进行修改"""
        if f_t_Nerf_dim is not None:
            in_dim = p_dim+f_t_Nerf_dim
            over_dim = in_dim
        else:
            in_dim = p_dim
            over_dim = in_dim
        ...
        for i in range(1, n_sigma_layers + 1): 
            ...
            in_dim = hid_dim if i + 1 not in skips else hid_dim + over_dim  # 这里也要改，这个是针对体特征传入的时候的情况
        self.sigma_layers = nn.ModuleList(sigma_layers)
        ...
        in_dim = hid_dim + d_dim    # 注意我们现在并没有在第二个颜色构建部分加入特征信息
        color_layers = []
        for _ in range(n_color_layers):
            color_layers.append(
    ...
    def forward(self, p, d=None, z=None, f_t_Nerf=None):
        """
        Args:
            p (float tensor, (bs, cx)): (embedded) point coordinates.
            d (float tensor, (bs, cd)): (embedded) ray directions.
            z (float tensor, (bs, cz)): (embedded) latent code.
            f_t_Nerf (float tensor, (bs, cf)): 我们自己加的一项，将volume feature也输入进来
        ...
        x = p
        if f_t_Nerf is not None: # 加的判断句，将特征融合进来
            x = torch.cat([p, f_t_Nerf],-1)
            print("f_t_Nerf is true")
            #print("f_t_Nerf is true, the concate shape is {}".format(x.shape))
            #print(f_t_Nerf.shape)
            #print(p.shape)
            p = x  # 这里p在下面是控制了残差网络传过去的东西，所以直接改为合成以后的结果
        for i in range(self.n_sigma_layers):
            if i + 1 in self.skips:
                x = torch.cat([x, p], -1)
            #print(self.sigma_layers[i])
            x = self.sigma_layers[i](x, gamma[k], beta[k])
            k += 1
        sigma = self.sigma_out(x)
...
class Renderer(nn.Module):
    ...
    def __init__(
        ...
        self.f_t_Nerf_embed_fn = f_t_Nerf_embed_fn  # 加入的体特征分析模块
    ...
    def evaluate_field(
        ...
        if z_vol is not None:
            ...
            z_sampled = z  #这里是我们自己加的，保存一下采样的结果
            if self.z_embed_fn is not None:
                z = self.z_embed_fn(z)                          # (bs*s, cz)

            #加入我们的模块
            if self.f_t_Nerf_embed_fn is not None:
                f_t_Nerf = self.f_t_Nerf_embed_fn(z_sampled)
        ...
        if self.p_embed_fn is not None:
            ...
            print("p 1 shape = {}".format(p.shape))
        p = p.repeat(bs, 1)                                     # (bs*s, cx)
        print("p 2 shape = {}".format(p.shape))
        ...
        sigma, color = self.field_fn(p, d, z, f_t_Nerf)                   # (bs*s, 1/3)
    ...
    def forward(
        ...
        """
        if rays.dim() == 4:
            rays = rays[0]  # 4的时候，第一个维度是gpu维度，也就是输入进来的东西是(g,1,n,7)(在给定n_ray的情况下)
        assert rays.dim() == 3
        assert rays.size(-1) == 7, 'ray bundle size mismatch'

        bs = z_vol.size(0) if z_vol is not None else 1
        v, r = rays.shape[:2] #给定n_ray时为(1,n,7)，否则是(v,r,7)，其实是一样的，在给定n_ray时候只有一个视角而已
        n = v * r               # number of rays PER SAMPLE

        #分别取出采样点坐标，采样方向，该光线的权重
        ...
        depths = steps.masked_select(valid)                             # (s,) 这个是pytorch中的方法，不知道为什么索引不到
        ...
        o = o.repeat_interleave(q, 0)                                   # (s, 3)重复和交错地插入元素以创建新的张量，
...
class TransientRenderer(Renderer):
    ...
    def sample_steps(self, o, d, n_steps=1):    #这里重载了父类中对应的部分
        """
...
class SteadyStateRenderer(Renderer):
    ...
    def __init__(
        ...
        z_embed_fn,             # embedding function for latent code
        field_fn,               # implicit volume function
        f_t_Nerf_embed_fn,      # 我们加入的体特征的分析模块
        bb_ctr=[0, 0, 0],       # bounding box center w.r.t. world frame
        bb_size=[2, 2, 2],      # bounding box size
        inf=10,                 # depth at infinity
        bin_len=0.03,           # distance covered by a bin
        ...
        super(SteadyStateRenderer, self).__init__(
            ...
            z_embed_fn=z_embed_fn, 
            field_fn=field_fn, 
            f_t_Nerf_embed_fn=f_t_Nerf_embed_fn,
            bb_ctr=bb_ctr, 
            bb_size=bb_size, 
            inf=inf,
            p_polar=p_polar, 
...
def make_renderer(config):
    ...
    # 这里建立我们自己的一个新的模块进来，这个是将提取到的特征也扔入到NeRF当中
    # 截止20230921，实现方法有两个思路，先按照pixelNeRF的方法去cat，如果能行的话就尝试用editing conditional radiance fields的方法
    if cf['embed_feature_to_NeRF'] is not None :
        if cf['embed_feature_to_NeRF']:
            f_t_Nerf_embed_fn = FeatureEmbedder(**cf['f_t_N'])  
            f_t_Nerf_dim = f_t_Nerf_embed_fn.out_dim
        else:
            f_t_Nerf_embed_fn = None
            f_t_Nerf_dim = None
    ...
    if cf['type'] == 'nerf':
        ...
        cf['nerf']['d_dim'] = d_dim
        cf['nerf']['z_dim'] = z_dim
        cf['nerf']['f_t_Nerf_dim'] = f_t_Nerf_dim #我们加的一项
    ...
    if config.get('transient') is not None:
        t_renderer = TransientRenderer( # 注意瞬态的我们没进行改正，还不能适用于加入体特征数据
    ...
    s_renderer = None
    """对他进行修改，使其能够满足我们的加入volume feature的要求"""
    if config.get('steady_state') is not None:  # 这个应该是直接渲染二维图像的那个渲染器
        s_renderer = SteadyStateRenderer(
            p_embed_fn, d_embed_fn, z_embed_fn, field_fn,f_t_Nerf_embed_fn,
