...
from .modules import *
from .ji_fk import *
#输出层修改为Hybrid Dilated Convolution（HDC)
# rsd与fk融合
...
class RSDEfficient(RSDBase):
    ...
    def forward(self, x, sqrt=True):
        ...
        ## Step 1: convert measurement into FDH
        fdata = fft.rfft(tdata, dim=1)                  # (bs*c, t//2+1, h, w)
        real_fdata = torch.real(fdata)
        imag_fdata = torch.imag(fdata)
        fdata = torch.complex(real_fdata[:, self.freq_idx],imag_fdata[:, self.freq_idx])
        #print("下界3")
        #print(self.freq_idx)
        #print(fdata.dtype)
        #print(fdata[:, self.freq_idx].dtype)
        #在实际训练中，fdata由于是complex64所以索引就会出错，报无法自动微分，就很烦
        #fdata = fdata[:, self.freq_idx]                 # (bs*c, o, h, w)
        ...
        if phase.dim() == 5:
            phase = torch.complex(phase[..., 0], phase[..., 1])
        print("执行RSD")
        print("fdsc的形状为{}".format(fsrc.shape))
        print("frsd的形状为{}".format(frsd.shape))
        temp = fsrc.unsqueeze(2)
        print("fsrc加一个维度的形状为{}".format(temp.shape))
        fdst = fsrc.unsqueeze(2) * frsd
        print("fdst的形状为{}".format(fdst.shape))
        # fdst = fsrc.unsqueeze(2) * self.frsd
        ...
        # 检查相关数值
        #has_nan = torch.isnan(tvol).any()
        #print(has_nan)  # 检查nan
        #has_inf = torch.isinf(tvol).any()
        #print(has_inf)  # 检查inf
        #print(tvol[0,0,:5,:5])

        # 这里有问题，torch1.7.1就tmd没有这个函数，自己写了一个拿来用
        tvol = ji_ifftshift(tvol, dim=(-2, -1))
        #tvol = torch.fft.ifftshift(tvol, dim=(-2, -1))
...
class RSDNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        #输出层修改为Hybrid Dilated Convolution（HDC) [1,2,5]，kernel=3的形式
        ...
        self.out_block = nn.Sequential(
            # dilation=1
            nn.Conv3d(in_plane, plane, kernel_size=3, stride=1, padding=1, bias=bias),
            make_norm3d(norm, plane, affine),
            make_actv(actv),
            # dilation=2
            nn.Conv3d(plane, plane, kernel_size=3, stride=1, padding=2, dilation=2, bias=bias),
            make_norm3d(norm, plane, affine),
            make_actv(actv),
            # dilation=5
            nn.Conv3d(plane, plane, kernel_size=3, stride=1, padding=5, dilation=5, bias=bias),
            make_norm3d(norm, plane, affine),
            make_actv(actv),
...
class FKNet(nn.Module):
    ...
    def __init__(
        ...
        in_plane=1,         # number of input planes
        plane=6,            # number of planes prior to propagation
        in_block=True,      # if True, learn conv block before FK
        ds=False,           # if True, down-sample the input
        fk_layer=None,     # FK kernel
        phasor_layer=None,
        actv="leaky_relu",  # activation function
        norm="none",        # normalization function
        affine=False,       # if True, apply learnable affine transform in norm
        **kwargs,
        ...
        self.phasor_layer = phasor_layer
        self.out_block = nn.Sequential(
            nn.Conv3d(in_plane, plane, 3, 1, 1, bias=bias),
            make_norm3d(norm, plane, affine),
            make_actv(actv),
        ...
        self.actv = make_actv("leaky_relu")
        self.norm = make_norm3d('batch',plane)
    def forward(self, x):
        x = self.in_block(x)
        ...
        #print(x.shape)
        tlen = x.shape[-3]
        #print("tbe = {},tlen = {}".format(tbe,tlen))
        x1 = self.fk_layer(x,[tbe,tbe,tbe],[tlen,tlen,tlen])
        x1 = self.norm(x1)
        x1 = self.actv(x1)

        x2 = self.phasor_layer(x,[tbe,tbe,tbe],[tlen,tlen,tlen])
        x2 = self.norm(x2)
        x2 = self.actv(x2)

        x = self.out_block((x1+x2)/2)
        return x
...
"""模仿他的写法，写一个我们自己的fk模块"""
def make_fk(config, plane, efficient=True):   #  建立FK模块
    fk = FK(**config)
    dev = "cuda"
    dnum = plane #这里是输入的特征的维数
    fk.todev(dev, dnum)
    #if efficient:
    #    fk = FKEfficient(**config)  # 高效FK
    #else:
    #    fk = FK(**config)  # 标准FK
    #if torch.cuda.is_available():
    #    fk.cuda()  # 将程序移动到GPU上进行并行计算
    return fk

def make_phasor(config, plane, efficient=True):   #  建立自己的Phasor模块
    fk = Phasor(**config)
    dev = "cuda"
    dnum = plane #这里是输入的特征的维数
    fk.todev(dev, dnum)
...
def make_fknet(config, efficient=True):
    # 我们自己加的
    fk = make_fk(config["fk"],config["plane"], efficient)
    phasor = make_phasor(config["fk"],config["plane"], efficient)
    fk_layer = fk
    phasor_layer = phasor
    fknet = FKNet(fk_layer=fk_layer, phasor_layer=phasor_layer, **config)
    return fknet
...
def make_encoder(config, efficient=True):
    ...
    if config["type"] == "rsd":
        ...
        encoder = make_rsdnet(config["rsdnet"], efficient)
            #加入fk的相关东西
    elif config["type"] == "fknet":
        encoder = make_fknet(config["fknet"], efficient)
